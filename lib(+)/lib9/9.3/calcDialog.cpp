...
#define BKSP 30
#define CLR 31
#define CLR_ALL 32// количество кнопок в группе, отображаемой в виде сетки
#define GRID_KEYS 16
/// Описатель кнопки
struct BtnDescr{
...
void InitBtnDescrArray()
{
    ...
    _btnDescr.push_back( BtnDescr("x^y", X_Y) );
    _btnDescr.push_back( BtnDescr("log x (y)", LOG) );
...
CalcDialog::CalcDialog(QWidget * parent)
{
    ...
    for (int i = 0; i < _btnDescr.size(); i++) 
    {
        ...
        if(i<GRID_KEYS)
        { 
            // Если кнопка из центрального блока - помещаем в сетку
            gridLayout->addWidget(button, i / 4, i % 4);
        }
        else if( i < GRID_KEYS+3) 
        {
            // кнопка из верхнего блока - в bccKeysLayout
        ...
        { 
            ...
            mainKeysLayout->addLayout(leftLayout);
...
{ 
    ...
    switch(id)
    {
        ...
        {
            // если на экране результат предыдущей операции - сброситьcheckOpPerf();
        ...
        default:
        {
            ...
            checkOpPerf();
            QString sId;// сформировать строку по идентификатору нажатой клавиши
            sId.setNum( id );
            QString str = m_pLineEdit->text ();
            if( str == "0" )
            {
